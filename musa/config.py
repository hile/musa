...
from musa.models import MusaDB, Setting, SyncTarget, Codec, Extension, Decoder, Encoder, Tree, Track
...
class MusaConfigDB(object):
    ...
    class MusaConfigInstance(MusaDB):
        ...
        def set(self, key, value):
            ...
            return self.session.add(Setting(key=key,value=value))
...
class CodecConfiguration(dict):
    ...
    def update_codec_description(self, name, description):
        codec = self.config.query(Codec).filter(name=name)
